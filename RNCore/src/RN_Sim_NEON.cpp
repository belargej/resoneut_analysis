...
#ifndef RNSIMNEON_CXX
#define RNSIMNEON_CXX
#include <numeric>
...
RN_Sim_NEON::RN_Sim_NEON(const std::string& name):RN_BaseClass(name),
					fTotEvents(0),
					fEDeposited(0),
					fNTOF(0),			    
					fNCM(0),
				 ...
				 
{
  ...
  void RN_Sim_NEON::LoadConfig(std::string input){    
    std::string temp;
...
void RN_Sim_NEON::Loop(){
  ...
  int RN_Sim_NEON::GenerateEvents(Long64_t evnum,TString options){
    Reset();
