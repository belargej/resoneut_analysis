...
void RN_EnergyLoss::Init(const std::string& projectile,const double& target,const std::string&option,const std::string& pressure,const double& EMax,Int_t steps){
  ...
  if (pPath!=NULL){
    //    printf ("The current path is: %s\n",pPath);
    filename = (std::string)pPath + "/input/SRIMTables/"+ projectile + "_" + Target_Name+"_" + option;
    if(option == "gas"){
      filename += "_" + pressure;
    }
    filename += ".in";
