...
bool ReconstructBeamSpotA::Begin(){
  ...
  bool ReconstructBeamSpotA::Process(){
    ...
    double ICEn = (ic.ERaw() + ic.DERaw())*0.0232919+11.874677;
    //std::cout << " IC Cal Mid   : " << ICCalEn << std::endl;
    ICEn = 41.87163*TMath::Power(TMath::E(),0.01108402*ICEn);
    //q_val_p = gReactionInfo.DecayQValueEstimate(si_array.E_AB(),useangle);
    if(UseInvMass)
      fErProton = gReactionInfo.IntHeavyQVal(si_array.E_AB(),ICEn,fSiBAngle*TMath::DegToRad(),si_array.Phi_B(),ic.Theta()*TMath::DegToRad(),ic.Phi()*TMath::DegToRad());
